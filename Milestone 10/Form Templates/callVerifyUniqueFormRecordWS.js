...
const webServiceName = 'VerifyUniqueFormRecordM10WS';
const formName = '10_Milestone';
        ...
        if (typeof resp.status != 'undefined') {
            ...
            if (resp.data[0] != undefined) {
                if (resp.data[0] == 'Success') {
                    message = resp.data[1] + 'so we are going to save ';

                    // HANDLE SUCCESS RESPONSE HERE

                    // Alway use .then for waiting for the form to save before running another function
                    //Save only if the status is Success because other way is not unique
