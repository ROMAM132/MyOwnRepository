...
module.exports.main = async function (ffCollection, vvClient, response) {
    ...
    */
    ...
    const templateName = 'City Lookup';
    const getCitiesByStateAndCounty = 'M11_SelectByParameters';
    /* -------------------------------------------------------------------------- */
    ...
    function updateFormData(formGUID) {
        ...
        const formFieldsToUpdate = {
            ['Field to Update']: `${currentDate.toString()}`,
        };
    ...
    try {
        ...
        const customQueryData = {
            sort: 'Zip Code', // sort field (tested with dates and strings)
            sortdir: 'asc',
            // params value must be a stringified array of objects with parameterName and value properties
            params: JSON.stringify([
                {
                    parameterName: 'stateParameter',
                    value: 'AZ',
                },
                {
                    parameterName: 'countyParameter',
                    value: 'Coconino',
                },
            ]),
        };
        ...
        const customQueryResp = await vvClient.customQuery
            .getCustomQueryResultsByName(getCitiesByStateAndCounty, customQueryData)
            .then((res) => parseRes(res))
            .then((res) => checkMetaAndStatus(res, shortDescription))
            .then((res) => checkDataPropertyExists(res, shortDescription))
            .then((res) => checkDataIsNotEmpty(res, shortDescription))
        ...
        const promiseAllData = await Promise.all(
            customQueryResp.map(async (product) => {
                const updatedFormData = await updateFormData(product.dhid);
        ...
        outputCollection[2] = promiseAllData;
